//A partir dela, execute os passos abaixo:

//a) Remova o excesso de espaços no final da string;

//b) exiba, em um console.log() a quantidade de caracteres da string, antes e depois da remoção dos espaços;

//) Substitua os traços `________` por “sticioso”.

const minhaString =
    "Eu não sou supersticioso, mas sou um pouco ________.      ",
  minhaNovaTrimmed = minhaString.trim(),
  minhaNovaString = minhaString.replace("________", "sticioso");

console.log(minhaNovaString, minhaString.length, minhaNovaTrimmed.length);
